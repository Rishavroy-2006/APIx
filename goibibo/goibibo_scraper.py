...
def parse_goibibo_page(page_text: str, origin: str, dest: str, date_str: str, advance_days: int, now_iso: str, capture_run: str) -> List[FareQuote]:
    """Parse Goibibo HTML / page source for flight quotes."""
    quotes = []
    ...
    if os.getenv("ENABLE_LLM_FALLBACK", "false").lower() == "true" and page_text:
        try:
            ...
            llm_quotes = llm_extract_flights(
                ...
                travel_date=date_str, advance_days=advance_days, source_scraper="goibibo"
            )
            if llm_quotes:
                return llm_quotes
    ...
    if not quotes:
        quotes.append(FareQuote(
            origin=origin, destination=dest, carrier_code="IB", carrier_name="Goibibo",
